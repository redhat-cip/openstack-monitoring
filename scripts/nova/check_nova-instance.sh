...
then
	...
	output_result "Unable to get tenant ID from Keystone API" $STATE_CRITICAL
fi
...
then
	...
	output_result "Unable to get image ID from nova API" $STATE_CRITICAL
fi
...
then
	...
	output_result "Unable to get flavor ID from nova API" $STATE_CRITICAL
fi
...
then
	...
	output_result "Unable to get instance ID from nova API" $STATE_CRITICAL
fi
...
then
	...
	output_result "Unable to get instance status from nova API" $STATE_CRITICAL
fi
...
if [ $TIME -gt 300 ]; then
	output_result "Unable to spawn instance quickly" $STATE_CRITICAL
else
	if [ $TIME -gt 180 ]; then
